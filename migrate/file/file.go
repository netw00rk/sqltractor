...
import (
	...
	"github.com/netw00rk/sqltractor/migrate/file/reader"
)

var filenameRegex = `^([0-9]+)_(.*)\.(up|down)\.%s$`

var defaultFileReader reader.FileReader
...
func ReadMigrationFiles(path string, filenameRegex *regexp.Regexp) (files MigrationFiles, err error) {
	...
	newFiles := make(MigrationFiles, len(tmpFiles))
	index := 0;
